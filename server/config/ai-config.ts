...
export const getAIConfig = (): AIServiceConfig => {
  ...
  const nutritionModel = process.env.AI_MODEL_NUTRITION || process.env.AI_MODEL || 'gpt-4o';
  const foodAnalysisModel = process.env.AI_MODEL_FOOD || process.env.AI_MODEL || 'gpt-4o';
  const programOptimizationModel = process.env.AI_MODEL_PROGRAM || process.env.AI_MODEL || 'gpt-4o';
  const multiImageModel = process.env.AI_MODEL_MULTI_IMAGE || process.env.AI_MODEL || 'gpt-4o';

  return {
    exerciseRecommendations: AI_MODELS[exerciseModel] || AI_MODELS['gpt-4o'],
    nutritionAnalysis: AI_MODELS[nutritionModel] || AI_MODELS['gpt-4o'],
    foodAnalysis: AI_MODELS[foodAnalysisModel] || AI_MODELS['gpt-4o'],
    programOptimization: AI_MODELS[programOptimizationModel] || AI_MODELS['gpt-4o'],
    multiImageNutrition: AI_MODELS[multiImageModel] || AI_MODELS['gpt-4o'],
  };
};
