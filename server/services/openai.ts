...
- STRICT LABEL READING: Read ONLY the exact values shown on the nutrition label. NEVER estimate or assume values. If label shows "Fiber 1g", report exactly 1g for fiber, not 5g or any other estimated value.
    ...
    if (userId) {
      ...
      result = await monitorAICall({
        ...
        const messages = [
          ...
          {
            role: "user", 
            content: "Correct. CRITICAL: For all nutrients with visible units on the label, use the {value: number, unit: string} format. Example: vitaminK: {value: 2600, unit: \"mcg\"}, vitaminC: {value: 10, unit: \"mg\"}. This format is MANDATORY for correct display. STRICT RULE: Read ONLY the exact values shown on the nutrition label - if label shows 'Fiber 1g', report exactly 1g, NOT 5g or any estimated value. Please proceed with the exact analysis, ensuring reported values match the label exactly AND include comprehensive micronutrients (minimum 40-80 nutrients) based on scientific nutritional databases."
          }
        ];
