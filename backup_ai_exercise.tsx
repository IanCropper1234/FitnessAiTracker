...
export default function CreateAIWorkoutSession() {
  ...
  const generateAITemplateName = async (exercises: ExerciseRecommendation[], isWeeklyPlan = false): Promise<string> => {
    try {
      const muscleGroups = Array.from(new Set(exercises.flatMap(ex => ex.muscleGroups))).slice(0, 3);
      const specialMethods = Array.from(new Set(exercises.filter(ex => ex.specialMethod && ex.specialMethod !== 'null').map(ex => ex.specialMethod)));
  ...
  const generateSmartFallbackName = (exercises: ExerciseRecommendation[], isWeeklyPlan = false): string => {
    const muscleGroups = Array.from(new Set(exercises.flatMap(ex => ex.muscleGroups))).slice(0, 2);
    const specialMethods = Array.from(new Set(exercises.filter(ex => ex.specialMethod && ex.specialMethod !== 'null').map(ex => ex.specialMethod)));
    ...
    <div className="min-h-screen bg-background p-4 space-y-6 ml-[-10px] mr-[-10px]">
      ...
      <Dialog open={showNameDialog} onOpenChange={setShowNameDialog}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Name Your Training Template</DialogTitle>
            <DialogDescription>
              AI has suggested a name, but you can customize it as needed.
            </DialogDescription>
          </DialogHeader>
