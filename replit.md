...
- **Custom Goal Persistence Fix (August 12, 2025)**: Resolved issue where custom calories and macro settings would revert to default values after successful submission. Enhanced diet-builder component to preserve user's custom settings through state management, preventing UI confusion about whether updates were successful. Added safeguards to prevent overriding user-set percentages during automatic data refresh.
- **Custom Goal UI Enhancement (August 12, 2025)**: Implemented redirect-based solution for custom goal submission. After saving custom macros and calories, users are automatically redirected to the integrated nutrition overview to view their updated settings. Added explanatory note clarifying that default values on custom goal page are TDEE-calculated and don't overwrite custom submissions.
